...
namespace Pokemmon.Buffs
{
	public class BuffZigzagoon : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Zigzagoon").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedZigzagoon = true;
				modPlayer.buffNormalType = true;
			}
			if (!modPlayer.summonedZigzagoon || modPlayer.pokemonAmount > 1) {
