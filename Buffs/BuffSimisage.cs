...
namespace Pokemmon.Buffs
{
	public class BuffSimisage : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Simisage").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedSimisage = true;
				modPlayer.buffGrassType = true;
			}
			if (!modPlayer.summonedSimisage || modPlayer.pokemonAmount > 1) {
