...
namespace Pokemmon.Buffs
{
	public class BuffGeodudeAlola : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("GeodudeAlola").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedGeodudeAlola = true;
				modPlayer.buffRockType = true;
				modPlayer.buffElectricType = true;
			}
			if (!modPlayer.summonedGeodudeAlola || modPlayer.pokemonAmount > 1) {
