...
namespace Pokemmon.Buffs
{
	public class BuffFurfrouDiamondTrim : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("FurfrouDiamondTrim").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedFurfrouDiamondTrim = true;
				modPlayer.buffNormalType = true;
			}
			if (!modPlayer.summonedFurfrouDiamondTrim || modPlayer.pokemonAmount > 1) {
