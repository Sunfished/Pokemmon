...
namespace Pokemmon.Buffs
{
	public class BuffMukAlola : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("MukAlola").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedMukAlola = true;
				modPlayer.buffPoisonType = true;
				modPlayer.buffDarkType = true;
			}
			if (!modPlayer.summonedMukAlola || modPlayer.pokemonAmount > 1) {
