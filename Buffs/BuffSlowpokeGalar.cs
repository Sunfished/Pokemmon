...
namespace Pokemmon.Buffs
{
	public class BuffSlowpokeGalar : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("SlowpokeGalar").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedSlowpokeGalar = true;
				modPlayer.buffPsychicType = true;
			}
			if (!modPlayer.summonedSlowpokeGalar || modPlayer.pokemonAmount > 1) {
