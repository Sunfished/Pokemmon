...
namespace Pokemmon.Buffs
{
	public class BuffFlorgesWhiteFlower : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("FlorgesWhiteFlower").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedFlorgesWhiteFlower = true;
				modPlayer.buffFairyType = true;
			}
			if (!modPlayer.summonedFlorgesWhiteFlower || modPlayer.pokemonAmount > 1) {
