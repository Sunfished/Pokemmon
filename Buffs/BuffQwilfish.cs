...
namespace Pokemmon.Buffs
{
	public class BuffQwilfish : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Qwilfish").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedQwilfish = true;
				modPlayer.buffWaterType = true;
				modPlayer.buffPoisonType = true;
			}
			if (!modPlayer.summonedQwilfish || modPlayer.pokemonAmount > 1) {
