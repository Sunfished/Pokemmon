...
namespace Pokemmon.Buffs
{
	public class BuffSimipour : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Simipour").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedSimipour = true;
				modPlayer.buffWaterType = true;
			}
			if (!modPlayer.summonedSimipour || modPlayer.pokemonAmount > 1) {
