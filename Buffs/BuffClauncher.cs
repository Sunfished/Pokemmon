...
namespace Pokemmon.Buffs
{
	public class BuffClauncher : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Clauncher").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedClauncher = true;
				modPlayer.buffWaterType = true;
			}
			if (!modPlayer.summonedClauncher || modPlayer.pokemonAmount > 1) {
