...
namespace Pokemmon.Buffs
{
	public class BuffSandaconda : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Sandaconda").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedSandaconda = true;
				modPlayer.buffGroundType = true;
			}
			if (!modPlayer.summonedSandaconda || modPlayer.pokemonAmount > 1) {
