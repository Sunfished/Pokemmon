...
namespace Pokemmon.Buffs
{
	public class BuffManectric : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Manectric").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedManectric = true;
				modPlayer.buffElectricType = true;
			}
			if (!modPlayer.summonedManectric || modPlayer.pokemonAmount > 1) {
