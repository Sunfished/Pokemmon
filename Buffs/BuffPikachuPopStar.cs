...
namespace Pokemmon.Buffs
{
	public class BuffPikachuPopStar : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("PikachuPopStar").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedPikachuPopStar = true;
				modPlayer.buffElectricType = true;
			}
			if (!modPlayer.summonedPikachuPopStar || modPlayer.pokemonAmount > 1) {
