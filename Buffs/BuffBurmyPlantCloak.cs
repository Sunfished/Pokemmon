...
namespace Pokemmon.Buffs
{
	public class BuffBurmyPlantCloak : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("BurmyPlantCloak").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedBurmyPlantCloak = true;
				modPlayer.buffBugType = true;
			}
			if (!modPlayer.summonedBurmyPlantCloak || modPlayer.pokemonAmount > 1) {
