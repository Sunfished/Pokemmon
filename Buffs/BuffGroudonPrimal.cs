...
namespace Pokemmon.Buffs
{
	public class BuffGroudonPrimal : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("GroudonPrimal").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedGroudonPrimal = true;
				modPlayer.buffGroundType = true;
				modPlayer.buffFireType = true;
			}
			if (!modPlayer.summonedGroudonPrimal || modPlayer.pokemonAmount > 1) {
