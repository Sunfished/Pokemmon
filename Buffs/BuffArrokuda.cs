...
namespace Pokemmon.Buffs
{
	public class BuffArrokuda : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Arrokuda").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedArrokuda = true;
				modPlayer.buffWaterType = true;
			}
			if (!modPlayer.summonedArrokuda || modPlayer.pokemonAmount > 1) {
