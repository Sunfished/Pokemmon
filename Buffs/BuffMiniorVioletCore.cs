...
namespace Pokemmon.Buffs
{
	public class BuffMiniorVioletCore : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("MiniorVioletCore").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedMiniorVioletCore = true;
				modPlayer.buffRockType = true;
				modPlayer.buffFlyingType = true;
			}
			if (!modPlayer.summonedMiniorVioletCore || modPlayer.pokemonAmount > 1) {
