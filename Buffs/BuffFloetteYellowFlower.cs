...
namespace Pokemmon.Buffs
{
	public class BuffFloetteYellowFlower : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("FloetteYellowFlower").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedFloetteYellowFlower = true;
				modPlayer.buffFairyType = true;
			}
			if (!modPlayer.summonedFloetteYellowFlower || modPlayer.pokemonAmount > 1) {
