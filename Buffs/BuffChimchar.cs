...
namespace Pokemmon.Buffs
{
	public class BuffChimchar : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Chimchar").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedChimchar = true;
				modPlayer.buffFireType = true;
			}
			if (!modPlayer.summonedChimchar || modPlayer.pokemonAmount > 1) {
