...
namespace Pokemmon.Buffs
{
	public class BuffPassimian : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Passimian").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedPassimian = true;
				modPlayer.buffFightingType = true;
			}
			if (!modPlayer.summonedPassimian || modPlayer.pokemonAmount > 1) {
