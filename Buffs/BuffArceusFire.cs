...
namespace Pokemmon.Buffs
{
	public class BuffArceusFire : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("ArceusFire").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedArceusFire = true;
				modPlayer.buffFireType = true;
			}
			if (!modPlayer.summonedArceusFire || modPlayer.pokemonAmount > 1) {
