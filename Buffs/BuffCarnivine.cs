...
namespace Pokemmon.Buffs
{
	public class BuffCarnivine : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Carnivine").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedCarnivine = true;
				modPlayer.buffGrassType = true;
			}
			if (!modPlayer.summonedCarnivine || modPlayer.pokemonAmount > 1) {
