...
namespace Pokemmon.Buffs
{
	public class BuffTypeNull : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("TypeNull").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedTypeNull = true;
				modPlayer.buffNormalType = true;
			}
			if (!modPlayer.summonedTypeNull || modPlayer.pokemonAmount > 1) {
