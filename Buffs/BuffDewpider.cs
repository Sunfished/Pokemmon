...
namespace Pokemmon.Buffs
{
	public class BuffDewpider : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Dewpider").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedDewpider = true;
				modPlayer.buffWaterType = true;
				modPlayer.buffBugType = true;
			}
			if (!modPlayer.summonedDewpider || modPlayer.pokemonAmount > 1) {
