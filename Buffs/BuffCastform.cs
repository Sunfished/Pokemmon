...
namespace Pokemmon.Buffs
{
	public class BuffCastform : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Castform").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedCastform = true;
				modPlayer.buffNormalType = true;
			}
			if (!modPlayer.summonedCastform || modPlayer.pokemonAmount > 1) {
