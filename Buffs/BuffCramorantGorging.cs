...
namespace Pokemmon.Buffs
{
	public class BuffCramorantGorging : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("CramorantGorging").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedCramorantGorging = true;
				modPlayer.buffFlyingType = true;
				modPlayer.buffWaterType = true;
			}
			if (!modPlayer.summonedCramorantGorging || modPlayer.pokemonAmount > 1) {
