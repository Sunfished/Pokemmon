...
namespace Pokemmon.Buffs
{
	public class BuffFlabebeBlueFlower : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("FlabebeBlueFlower").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedFlabebeBlueFlower = true;
				modPlayer.buffFairyType = true;
			}
			if (!modPlayer.summonedFlabebeBlueFlower || modPlayer.pokemonAmount > 1) {
