...
namespace Pokemmon.Buffs
{
	public class BuffNidorino : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Nidorino").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedNidorino = true;
				modPlayer.buffPoisonType = true;
			}
			if (!modPlayer.summonedNidorino || modPlayer.pokemonAmount > 1) {
