...
namespace Pokemmon.Buffs
{
	public class BuffDeerlingWinter : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("DeerlingWinter").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedDeerlingWinter = true;
				modPlayer.buffNormalType = true;
				modPlayer.buffGrassType = true;
			}
			if (!modPlayer.summonedDeerlingWinter || modPlayer.pokemonAmount > 1) {
