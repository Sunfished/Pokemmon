...
namespace Pokemmon.Buffs
{
	public class BuffTyphlosion : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Typhlosion").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedTyphlosion = true;
				modPlayer.buffFireType = true;
			}
			if (!modPlayer.summonedTyphlosion || modPlayer.pokemonAmount > 1) {
