...
namespace Pokemmon.Buffs
{
	public class BuffCyndaquil : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Cyndaquil").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedCyndaquil = true;
				modPlayer.buffFireType = true;
			}
			if (!modPlayer.summonedCyndaquil || modPlayer.pokemonAmount > 1) {
