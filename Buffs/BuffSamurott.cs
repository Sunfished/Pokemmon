...
namespace Pokemmon.Buffs
{
	public class BuffSamurott : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Samurott").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedSamurott = true;
				modPlayer.buffWaterType = true;
			}
			if (!modPlayer.summonedSamurott || modPlayer.pokemonAmount > 1) {
