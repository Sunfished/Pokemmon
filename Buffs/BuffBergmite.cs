...
namespace Pokemmon.Buffs
{
	public class BuffBergmite : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Bergmite").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedBergmite = true;
				modPlayer.buffIceType = true;
			}
			if (!modPlayer.summonedBergmite || modPlayer.pokemonAmount > 1) {
