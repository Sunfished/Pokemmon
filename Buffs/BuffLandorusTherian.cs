...
namespace Pokemmon.Buffs
{
	public class BuffLandorusTherian : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("LandorusTherian").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedLandorusTherian = true;
				modPlayer.buffGroundType = true;
				modPlayer.buffFlyingType = true;
			}
			if (!modPlayer.summonedLandorusTherian || modPlayer.pokemonAmount > 1) {
