...
namespace Pokemmon.Buffs
{
	public class BuffSilvallyFighting : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("SilvallyFighting").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedSilvallyFighting = true;
				modPlayer.buffFightingType = true;
			}
			if (!modPlayer.summonedSilvallyFighting || modPlayer.pokemonAmount > 1) {
