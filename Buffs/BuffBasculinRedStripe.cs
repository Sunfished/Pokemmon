...
namespace Pokemmon.Buffs
{
	public class BuffBasculinRedStripe : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[mod.ProjectileType("BasculinRedStripe")] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedBasculinRedStripe = true;
				modPlayer.buffWaterType = true;
			}
			if (!modPlayer.summonedBasculinRedStripe || modPlayer.pokemonAmount > 1) {
