...
namespace Pokemmon.Buffs
{
	public class BuffChingling : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Chingling").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedChingling = true;
				modPlayer.buffPsychicType = true;
			}
			if (!modPlayer.summonedChingling || modPlayer.pokemonAmount > 1) {
