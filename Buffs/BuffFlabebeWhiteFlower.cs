...
namespace Pokemmon.Buffs
{
	public class BuffFlabebeWhiteFlower : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("FlabebeWhiteFlower").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedFlabebeWhiteFlower = true;
				modPlayer.buffFairyType = true;
			}
			if (!modPlayer.summonedFlabebeWhiteFlower || modPlayer.pokemonAmount > 1) {
