...
namespace Pokemmon.Buffs
{
	public class BuffPikachuOriginalCap : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("PikachuOriginalCap").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedPikachuOriginalCap = true;
				modPlayer.buffElectricType = true;
			}
			if (!modPlayer.summonedPikachuOriginalCap || modPlayer.pokemonAmount > 1) {
