...
namespace Pokemmon.Buffs
{
	public class BuffFurfrouLaReineTrim : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("FurfrouLaReineTrim").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedFurfrouLaReineTrim = true;
				modPlayer.buffNormalType = true;
			}
			if (!modPlayer.summonedFurfrouLaReineTrim || modPlayer.pokemonAmount > 1) {
