...
namespace Pokemmon.Buffs
{
	public class BuffCastformSnowy : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("CastformSnowy").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedCastformSnowy = true;
				modPlayer.buffIceType = true;
			}
			if (!modPlayer.summonedCastformSnowy || modPlayer.pokemonAmount > 1) {
