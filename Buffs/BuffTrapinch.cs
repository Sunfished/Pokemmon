...
namespace Pokemmon.Buffs
{
	public class BuffTrapinch : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Trapinch").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedTrapinch = true;
				modPlayer.buffGroundType = true;
			}
			if (!modPlayer.summonedTrapinch || modPlayer.pokemonAmount > 1) {
