...
namespace Pokemmon.Buffs
{
	public class BuffMiniorBlueCore : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("MiniorBlueCore").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedMiniorBlueCore = true;
				modPlayer.buffRockType = true;
				modPlayer.buffFlyingType = true;
			}
			if (!modPlayer.summonedMiniorBlueCore || modPlayer.pokemonAmount > 1) {
