...
namespace Pokemmon.Buffs
{
	public class BuffSilvallyFlying : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("SilvallyFlying").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedSilvallyFlying = true;
				modPlayer.buffFlyingType = true;
			}
			if (!modPlayer.summonedSilvallyFlying || modPlayer.pokemonAmount > 1) {
