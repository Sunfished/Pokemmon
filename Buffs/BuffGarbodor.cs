...
namespace Pokemmon.Buffs
{
	public class BuffGarbodor : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Garbodor").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedGarbodor = true;
				modPlayer.buffPoisonType = true;
			}
			if (!modPlayer.summonedGarbodor || modPlayer.pokemonAmount > 1) {
