...
namespace Pokemmon.Buffs
{
	public class BuffArceusSteel : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("ArceusSteel").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedArceusSteel = true;
				modPlayer.buffSteelType = true;
			}
			if (!modPlayer.summonedArceusSteel || modPlayer.pokemonAmount > 1) {
