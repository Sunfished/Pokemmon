...
namespace Pokemmon.Buffs
{
	public class BuffZebstrika : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Zebstrika").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedZebstrika = true;
				modPlayer.buffElectricType = true;
			}
			if (!modPlayer.summonedZebstrika || modPlayer.pokemonAmount > 1) {
