...
namespace Pokemmon.Buffs
{
	public class BuffToxtricityAmped : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("ToxtricityAmped").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedToxtricityAmped = true;
				modPlayer.buffPoisonType = true;
				modPlayer.buffElectricType = true;
			}
			if (!modPlayer.summonedToxtricityAmped || modPlayer.pokemonAmount > 1) {
