...
namespace Pokemmon.Buffs
{
	public class BuffRookidee : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Rookidee").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedRookidee = true;
				modPlayer.buffFlyingType = true;
			}
			if (!modPlayer.summonedRookidee || modPlayer.pokemonAmount > 1) {
