...
{
	public class BuffSnom : ModBuff
	{
		public override void SetStaticDefaults() {
			DisplayName.SetDefault("Go, Snom!");
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Snom").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedSnom = true;
				modPlayer.buffIceType = true;
				modPlayer.buffBugType = true;
			}
			if (!modPlayer.summonedSnom || modPlayer.pokemonAmount > 1) {
