...
namespace Pokemmon.Buffs
{
	public class BuffKricketot : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Kricketot").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedKricketot = true;
				modPlayer.buffBugType = true;
			}
			if (!modPlayer.summonedKricketot || modPlayer.pokemonAmount > 1) {
