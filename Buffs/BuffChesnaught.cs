...
namespace Pokemmon.Buffs
{
	public class BuffChesnaught : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Chesnaught").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedChesnaught = true;
				modPlayer.buffGrassType = true;
				modPlayer.buffFightingType = true;
			}
			if (!modPlayer.summonedChesnaught || modPlayer.pokemonAmount > 1) {
