...
namespace Pokemmon.Buffs
{
	public class BuffGenesectDouseDrive : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("GenesectDouseDrive").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedGenesectDouseDrive = true;
				modPlayer.buffBugType = true;
				modPlayer.buffSteelType = true;
			}
			if (!modPlayer.summonedGenesectDouseDrive || modPlayer.pokemonAmount > 1) {
