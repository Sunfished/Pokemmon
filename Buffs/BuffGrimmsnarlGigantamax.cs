...
namespace Pokemmon.Buffs
{
	public class BuffGrimmsnarlGigantamax : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("GrimmsnarlGigantamax").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedGrimmsnarlGigantamax = true;
				modPlayer.buffDarkType = true;
				modPlayer.buffFairyType = true;
			}
			if (!modPlayer.summonedGrimmsnarlGigantamax || modPlayer.pokemonAmount > 1) {
