...
namespace Pokemmon.Buffs
{
	public class BuffGenesectChillDrive : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("GenesectChillDrive").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedGenesectChillDrive = true;
				modPlayer.buffBugType = true;
				modPlayer.buffSteelType = true;
			}
			if (!modPlayer.summonedGenesectChillDrive || modPlayer.pokemonAmount > 1) {
