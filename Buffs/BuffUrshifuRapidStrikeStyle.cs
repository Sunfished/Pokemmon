...
namespace Pokemmon.Buffs
{
	public class BuffUrshifuRapidStrikeStyle : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("UrshifuRapidStrikeStyle").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedUrshifuRapidStrikeStyle = true;
				modPlayer.buffFightingType = true;
				modPlayer.buffWaterType = true;
			}
			if (!modPlayer.summonedUrshifuRapidStrikeStyle || modPlayer.pokemonAmount > 1) {
