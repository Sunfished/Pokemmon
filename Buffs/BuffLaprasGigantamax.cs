...
namespace Pokemmon.Buffs
{
	public class BuffLaprasGigantamax : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("LaprasGigantamax").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedLaprasGigantamax = true;
				modPlayer.buffWaterType = true;
				modPlayer.buffIceType = true;
			}
			if (!modPlayer.summonedLaprasGigantamax || modPlayer.pokemonAmount > 1) {
