...
namespace Pokemmon.Buffs
{
	public class BuffSilvallyDark : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("SilvallyDark").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedSilvallyDark = true;
				modPlayer.buffDarkType = true;
			}
			if (!modPlayer.summonedSilvallyDark || modPlayer.pokemonAmount > 1) {
