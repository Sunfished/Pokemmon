...
namespace Pokemmon.Buffs
{
	public class BuffFurfrouStarTrim : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("FurfrouStarTrim").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedFurfrouStarTrim = true;
				modPlayer.buffNormalType = true;
			}
			if (!modPlayer.summonedFurfrouStarTrim || modPlayer.pokemonAmount > 1) {
