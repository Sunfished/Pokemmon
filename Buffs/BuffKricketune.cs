...
namespace Pokemmon.Buffs
{
	public class BuffKricketune : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Kricketune").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedKricketune = true;
				modPlayer.buffBugType = true;
			}
			if (!modPlayer.summonedKricketune || modPlayer.pokemonAmount > 1) {
