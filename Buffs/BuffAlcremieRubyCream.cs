...
namespace Pokemmon.Buffs
{
	public class BuffAlcremieRubyCream : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("AlcremieRubyCream").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedAlcremieRubyCream = true;
				modPlayer.buffFairyType = true;
			}
			if (!modPlayer.summonedAlcremieRubyCream || modPlayer.pokemonAmount > 1) {
