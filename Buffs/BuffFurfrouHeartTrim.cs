...
namespace Pokemmon.Buffs
{
	public class BuffFurfrouHeartTrim : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("FurfrouHeartTrim").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedFurfrouHeartTrim = true;
				modPlayer.buffNormalType = true;
			}
			if (!modPlayer.summonedFurfrouHeartTrim || modPlayer.pokemonAmount > 1) {
