...
namespace Pokemmon.Buffs
{
	public class BuffRegigigas : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Regigigas").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedRegigigas = true;
				modPlayer.buffNormalType = true;
			}
			if (!modPlayer.summonedRegigigas || modPlayer.pokemonAmount > 1) {
