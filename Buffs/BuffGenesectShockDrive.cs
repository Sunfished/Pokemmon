...
namespace Pokemmon.Buffs
{
	public class BuffGenesectShockDrive : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("GenesectShockDrive").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedGenesectShockDrive = true;
				modPlayer.buffBugType = true;
				modPlayer.buffSteelType = true;
			}
			if (!modPlayer.summonedGenesectShockDrive || modPlayer.pokemonAmount > 1) {
