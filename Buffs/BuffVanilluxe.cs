...
namespace Pokemmon.Buffs
{
	public class BuffVanilluxe : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Vanilluxe").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedVanilluxe = true;
				modPlayer.buffIceType = true;
			}
			if (!modPlayer.summonedVanilluxe || modPlayer.pokemonAmount > 1) {
