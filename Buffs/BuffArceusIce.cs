...
namespace Pokemmon.Buffs
{
	public class BuffArceusIce : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("ArceusIce").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedArceusIce = true;
				modPlayer.buffIceType = true;
			}
			if (!modPlayer.summonedArceusIce || modPlayer.pokemonAmount > 1) {
