...
namespace Pokemmon.Buffs
{
	public class BuffDarmanitan : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Darmanitan").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedDarmanitan = true;
				modPlayer.buffFireType = true;
			}
			if (!modPlayer.summonedDarmanitan || modPlayer.pokemonAmount > 1) {
