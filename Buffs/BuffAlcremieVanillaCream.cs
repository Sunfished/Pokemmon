...
namespace Pokemmon.Buffs
{
	public class BuffAlcremieVanillaCream : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("AlcremieVanillaCream").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedAlcremieVanillaCream = true;
				modPlayer.buffFairyType = true;
			}
			if (!modPlayer.summonedAlcremieVanillaCream || modPlayer.pokemonAmount > 1) {
