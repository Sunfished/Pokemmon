...
namespace Pokemmon.Buffs
{
	public class BuffArceusFighting : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("ArceusFighting").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedArceusFighting = true;
				modPlayer.buffFightingType = true;
			}
			if (!modPlayer.summonedArceusFighting || modPlayer.pokemonAmount > 1) {
