...
namespace Pokemmon.Buffs
{
	public class BuffCroconaw : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Croconaw").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedCroconaw = true;
				modPlayer.buffWaterType = true;
			}
			if (!modPlayer.summonedCroconaw || modPlayer.pokemonAmount > 1) {
