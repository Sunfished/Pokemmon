...
namespace Pokemmon.Buffs
{
	public class BuffAlcremieMintCream : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("AlcremieMintCream").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedAlcremieMintCream = true;
				modPlayer.buffFairyType = true;
			}
			if (!modPlayer.summonedAlcremieMintCream || modPlayer.pokemonAmount > 1) {
