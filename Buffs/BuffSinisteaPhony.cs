...
namespace Pokemmon.Buffs
{
	public class BuffSinisteaPhony : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("SinisteaPhony").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedSinisteaPhony = true;
				modPlayer.buffGhostType = true;
			}
			if (!modPlayer.summonedSinisteaPhony || modPlayer.pokemonAmount > 1) {
