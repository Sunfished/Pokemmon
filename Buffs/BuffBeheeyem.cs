...
namespace Pokemmon.Buffs
{
	public class BuffBeheeyem : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Beheeyem").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedBeheeyem = true;
				modPlayer.buffPsychicType = true;
			}
			if (!modPlayer.summonedBeheeyem || modPlayer.pokemonAmount > 1) {
