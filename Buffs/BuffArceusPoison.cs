...
namespace Pokemmon.Buffs
{
	public class BuffArceusPoison : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("ArceusPoison").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedArceusPoison = true;
				modPlayer.buffPoisonType = true;
			}
			if (!modPlayer.summonedArceusPoison || modPlayer.pokemonAmount > 1) {
