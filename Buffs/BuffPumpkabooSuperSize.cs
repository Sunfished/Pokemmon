...
namespace Pokemmon.Buffs
{
	public class BuffPumpkabooSuperSize : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("PumpkabooSuperSize").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedPumpkabooSuperSize = true;
				modPlayer.buffGhostType = true;
				modPlayer.buffGrassType = true;
			}
			if (!modPlayer.summonedPumpkabooSuperSize || modPlayer.pokemonAmount > 1) {
