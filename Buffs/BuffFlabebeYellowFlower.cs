...
namespace Pokemmon.Buffs
{
	public class BuffFlabebeYellowFlower : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("FlabebeYellowFlower").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedFlabebeYellowFlower = true;
				modPlayer.buffFairyType = true;
			}
			if (!modPlayer.summonedFlabebeYellowFlower || modPlayer.pokemonAmount > 1) {
