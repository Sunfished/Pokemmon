...
namespace Pokemmon.Buffs
{
	public class BuffDelcatty : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Delcatty").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedDelcatty = true;
				modPlayer.buffNormalType = true;
			}
			if (!modPlayer.summonedDelcatty || modPlayer.pokemonAmount > 1) {
