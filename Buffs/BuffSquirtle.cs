...
namespace Pokemmon.Buffs
{
	public class BuffSquirtle : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Squirtle").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedSquirtle = true;
				modPlayer.buffWaterType = true;
			}
			if (!modPlayer.summonedSquirtle || modPlayer.pokemonAmount > 1) {
