...
namespace Pokemmon.Buffs
{
	public class BuffNidoranM : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("NidoranM").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedNidoranM = true;
				modPlayer.buffPoisonType = true;
			}
			if (!modPlayer.summonedNidoranM || modPlayer.pokemonAmount > 1) {
