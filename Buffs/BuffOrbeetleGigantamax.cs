...
namespace Pokemmon.Buffs
{
	public class BuffOrbeetleGigantamax : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("OrbeetleGigantamax").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedOrbeetleGigantamax = true;
				modPlayer.buffBugType = true;
				modPlayer.buffPsychicType = true;
			}
			if (!modPlayer.summonedOrbeetleGigantamax || modPlayer.pokemonAmount > 1) {
