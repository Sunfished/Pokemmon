...
namespace Pokemmon.Buffs
{
	public class BuffSmogecko : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex) {
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("Smogecko").Type] > 0 && modPlayer.pokemonAmount == 1) {
				player.buffTime[buffIndex] = 18000;
				modPlayer.summonedSmogecko = true;
				modPlayer.buffFireType = true;
			}
			if (!modPlayer.summonedSmogecko || modPlayer.pokemonAmount > 1) {
